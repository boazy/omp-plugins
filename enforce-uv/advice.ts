  ...
  switch (v.kind) {
    ...
    case "uv-pip":
      return [
        ...
        "  • Live/stateful interpreter on an isolated venv (e.g. omp's eval IPython",
        "    kernel using a project `.venv`) → this IS the right tool; there's no file",
        "    to declare deps in. Best done *inside* the kernel — `!uv pip install <pkg>`",
        "    in a `py` cell (or `%pip install <pkg>` if uv is unavailable), which isn't",
        "    guarded. Make sure the kernel isn't on system Python first. From bash, override:",
        "",
